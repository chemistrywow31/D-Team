---
name: Worklog
description: Maintain phase-level documentation with evidence chain for all tasks
---
...
- `{task-name}`: kebab-case (e.g., `add-user-auth`)
- `phase-{n}-{label}`: numbered sequentially, kebab-case label (e.g., `phase-1-spec`)
...
- File paths to internal documents referenced (including OpenSpec artifacts)
...
- Micro tasks (< 30 min) using `/gsd:quick` do not require a worklog
